...
export default () => {
  ...
  useEffect(() => {
    ...
    fetch("https://bfstats-api.leonlarsson.com")
      .then(res => res.json())
      .then(json => setData(<Fragment>In <b>{json.totalGuilds.toLocaleString("en-US")}</b> servers, with <b>{json.totalMembers.toLocaleString("en-US")}</b> members, and <b>{json.totalStatsSent.total.toLocaleString("en-US")}</b> stats sent.</Fragment>))
  }, []);
