...
**Character:** One neutral, engineered grotesk doing all the work, distinguished only by weight and size. Geist is deliberate here and is not up for revision: it is the incumbent face, self-hosted, and its tabular figures and tight default tracking are what let dense stat columns and live counters hold alignment as they tick. A generic design detector will flag Geist as an "overused font"; that warning is a knowing, brief-sanctioned override and must not be "fixed" by a later pass.
